...
class EscapeParser {
  ...
  bool _consumeCsi() {
    ...
    while (true) {
      ...
      // ':' (colon) is the ITU-T T.416 sub-parameter separator used by many
      // modern terminal apps (e.g. fzf) for extended SGR colors, such as
      // `38:5:196` (256-color) or `38:2::255:0:0` (truecolor). It was
      // previously matched by none of the branches below, so the byte was
      // silently dropped without resetting the accumulator - corrupting the
      // parsed integer and breaking every colon-delimited color/attribute
      // sequence. Treat it the same as ';' so it still splits parameters.
